...
build-all: build/kubernetes-eventexporter

build/kubernetes-eventexporter: FORCE
	@env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/kubernetes-eventexporter .
...
install: FORCE build/kubernetes-eventexporter
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/kubernetes-eventexporter "$(DESTDIR)$(PREFIX)/bin/kubernetes-eventexporter"
...
goimports: FORCE
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/kubernetes-eventexporter\e[0m\n"
	@goimports -w -local https://github.com/sapcc/kubernetes-eventexporter internal/ $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/kubernetes-eventexporter\e[0m  Build kubernetes-eventexporter.\n"
